 ...
 */
function initializeFirebase(adminInstance, overrideConfig, protectProduction) {
    try {
        ...
        else if (protectProduction &&
            protectProduction.rtdb &&
            protectProduction.rtdb !== 'none') {
            // biome-ignore lint/suspicious/noConsole: Intentional logging
            console.warn('cypress-firebase: FIREBASE_DATABASE_EMULATOR_HOST is not set, RTDB operations may alter production instead of emulator!');
            if (protectProduction.rtdb === 'error') {
                throw new Error('cypress-firebase: FIREBASE_DATABASE_EMULATOR_HOST is not set. Set FIREBASE_DATABASE_EMULATOR_HOST environment variable or change protectProduction.rtdb setting');
            }
        }
        if (process.env.FIREBASE_AUTH_EMULATOR_HOST) {
            ...
            // biome-ignore lint/suspicious/noConsole: Intentional logging
            console.warn('cypress-firebase: FIREBASE_AUTH_EMULATOR_HOST is not set, auth operations may alter production instead of emulator!');
            if (protectProduction.auth === 'error') {
                throw new Error('cypress-firebase: FIREBASE_AUTH_EMULATOR_HOST is not set. Set FIREBASE_AUTH_EMULATOR_HOST environment variable or change protectProduction.auth setting');
            }
        }
        ...
        if (!fbConfig.projectId) {
            const projectId = process.env.GCLOUD_PROJECT ||
                ((fbConfig && fbConfig.credential) || {}).projectId;
            if (projectId) {
        ...
        if (process.env.FIRESTORE_EMULATOR_HOST) {
            ...
            // biome-ignore lint/suspicious/noConsole: Intentional logging
            console.warn('cypress-firebase: FIRESTORE_EMULATOR_HOST is not set, Firestore operations may alter production instead of emulator!');
            if (protectProduction.firestore === 'error') {
                throw new Error('cypress-firebase: FIRESTORE_EMULATOR_HOST is not set. Set FIRESTORE_EMULATOR_HOST environment variable or change protectProduction.firestore setting');
            }
        }
...
function slashPathToFirestoreRef(firestoreStatics, slashPath, options) {
    ...
    if (options &&
        ...
        if (Array.isArray(options.where[0])) {
            options.where.forEach((whereCondition) => {
