...
export default function Chatbot({ 
  ...
  return (
    <>
      ...
      {/* Chat Window - use 100dvh when fullscreen on mobile so input stays visible (100vh can overscroll on mobile Safari) */}
      {isOpen && (
        <div 
          className={cn(
          ...
          style={{
            ...
            height: isFullscreen && (responsive.isMobile || responsive.isTablet)
        ...
        >
          {/* Header - flex-shrink-0 so input area stays visible when extended/fullscreen */}
          <div className={cn(
            "flex-shrink-0 border-b border-neutral-200 bg-white flex items-center justify-between",
            spacing.padding
          )}>
          ...
          {/* Messages - min-h-0 allows flex child to shrink so input stays on screen when extended */}
          ...
          {/* Input Area - flex-shrink-0 so it's always visible when extended/fullscreen on mobile */}
          <div className={cn(
            "flex-shrink-0 border-t border-neutral-200 bg-white",
            spacing.padding,
            (responsive.isMobile || responsive.isTablet) && isFullscreen && "pb-[env(safe-area-inset-bottom)]"
          )}>
            <form
              onSubmit={(e) => {
                e.preventDefault();
