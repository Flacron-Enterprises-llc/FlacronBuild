...
const projectSchema = z.object({
  ...
  area: z.number().min(1, "Area must be at least 1 sq ft").optional(),
...
export default function EstimationForm({ userRole, onEstimateGenerated, onReportSaved, disableRoleSelection, onFieldFocus, hasEstimate }: EstimationFormProps) {
  ...
  const createProjectMutation = useMutation({
    mutationFn: async (data: ProjectFormData) => {
      // Use form area if provided, otherwise default minimum for roofing projects
      const estimatedArea = (data.area != null && data.area >= 1) ? data.area : Math.max(800, 1000);
      ...
      const projectData = {
        ...
        area: estimatedArea, // From form or default
  ...
  const onSubmit = (data: ProjectFormData) => {
    ...
    // Currency is not required for insurance adjusters or inspectors (their reports don't display currency)
    const requiredFields = ['name', 'userRole', 'projectType', 'location', 'structureType', 'roofPitch', 'roofAge', 'materialLayers', 'preferredLanguage'];
    if (formData.userRole !== 'insurance-adjuster' && formData.userRole !== 'inspector') {
                   ...
                   {selectedRole === "inspector" && (
                     <div className="space-y-4">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         ...
                         <FormField
                           control={form.control}
                           name="inspectorInfo.contact"
                           render={({ field }) => (
                             <FormItem>
                               <FormLabel>Contact (email or phone)</FormLabel>
                               <FormControl>
                                 <Input placeholder="Email or phone" {...field} onFocus={() => handleFieldFocus('inspectorInfo.contact')} />
                               </FormControl>
                               <FormMessage />
                             </FormItem>
                           )}
                       ...
                       {/* Report language only — inspector report does not display currency */}
                       <FormField
                         control={form.control}
                           ...
                           render={({ field }) => (
                             <FormItem>
                               ...
                               >
                                 <FormControl>
                                   <SelectTrigger>
                                     <SelectValue placeholder="Select weather at time of inspection" />
                                   </SelectTrigger>
                                 </FormControl>
                                 <SelectContent>
                                   <SelectItem value="clear-sunny">Clear and Sunny</SelectItem>
